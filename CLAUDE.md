...
- **MMD4B panel**: N-panel (tab "MMD4B"). Main panel: model name, Emission slider, Outline Thickness + Build/Rebuild/Remove (one row), SDEF info + Bake (inline, when model has SDEF). Sub-panels (collapsed by default): Physics (NCC mode, Build/Reset/Rebuild NCCs/Remove, selected RB inspect, per-chain toggles), IK (per-chain toggles + All On/Off), Animation (actions, NLA, Mark as Assets, Remove Animation). Mesh sub-panel (open by default, only when mesh child selected): per-mesh Emission Factor, outline toggle/color/factor, SDEF vertex count + Select, physics chains, Delete Mesh.
- **Materials**: Two shader modes controlled by "Toon & Sphere Textures" checkbox on PMX import (off by default):
  ...
  - **Overlapping materials** (auto-detected): Diffuse BSDF + Transparent BSDF → Mix Shader (fac = alpha). BLENDED render method, `use_transparency_overlap=False`. Prevents z-fighting on eye highlights, face layers, etc. Detection runs on full PMX face data before mesh split — faces sharing vertex positions across materials flag the later material.
