...
- **Milestone 4b** (done): Physics cleanup — two modes (none, rigid_body), MMD4B panel with Build/Rebuild/Clear. Collision quality (High/Draft), per-chain collision/physics/self-collision toggles, NCC rebuild. Cloth/soft body deferred to future phase.
...
- **Physics**: Two modes:
  ...
  - `rigid_body`: RBW disabled during build, collision layers (shared layer 0 + own group), non-collision constraint (NCC) empties for all excluded pairs (no proximity filter — correctness over count), all joints `disable_collisions=True`, margin 1e-6, dynamic body repositioning, depsgraph flushes. Build restructured into 3 phases (CREATE → POSITION → COUPLE & ACTIVATE). Blender's `collision_collections` is symmetric (shared layer = collide) while PMX masks are asymmetric (bilateral check: `A.group & B.mask && B.group & A.mask`), so NCC empties are required for correct exclusion.
  - **Collision quality** (stored on armature): `high` (default) = full NCC empties, `draft` = no collisions (bodies pass through each other, springs/joints still work). Draft is useful for fast preview.
  - **Per-chain controls**: Each chain has independent collision toggle (eye icon — sets `collision_collections=[False]*20`), physics toggle (physics icon — sets `kinematic=True`), and self-collision toggle (link icon — skips intra-chain NCC empties). Collision and physics toggles are instant (no rebuild). Self-collision toggle triggers NCC rebuild. Settings stored as `mmd_chain_collision_disabled`, `mmd_chain_physics_disabled`, `mmd_chain_self_collision_disabled` JSON on armature — preserved across clear/rebuild.
  - **NCC rebuild**: `rebuild_ncc()` reuses existing NCC empties by reassigning pairs, only creating/deleting the difference. Respects self-collision and disabled chains. ~0.1s for 13K empties.
...
- **MMD4B panel**: N-panel (tab "MMD4B") with sub-panels: Physics (Quality dropdown, Build/Reset/Rebuild NCCs/Remove, selected RB info with Inspect/Colliders/Contacts, per-chain list with collision/physics/self-collision toggles), IK Toggle (per-chain toggles + All On/Off), Animation (action name, Clear Animation).
