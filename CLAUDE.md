...
- **Milestone 4b** (done): Physics cleanup — two modes (none, rigid_body), MMD4B panel with Build/Rebuild/Clear. Collision quality (High/Draft), per-chain collision/physics toggles, chain exclusions to reduce NCC count, NCC rebuild. Cloth/soft body deferred to future phase.
...
- **Physics**: Two modes:
  ...
  - **Per-chain controls**: Each chain has independent collision toggle (eye icon — sets `collision_collections=[False]*20`) and physics toggle (physics icon — sets `kinematic=True`). Both are instant (no rebuild). Per-chain exclusions (filter icon) skip NCC empties between mutually excluded chains (takes effect on NCC rebuild). Settings stored as `mmd_chain_collision_disabled`, `mmd_chain_physics_disabled`, `mmd_chain_exclusions` JSON on armature — preserved across clear/rebuild.
  - **NCC rebuild**: `rebuild_ncc()` reuses existing NCC empties by reassigning pairs, only creating/deleting the difference. Respects chain exclusions and disabled chains. ~0.1s for 13K empties. Useful after changing exclusion settings.
...
- **MMD4B panel**: N-panel (tab "MMD4B") with sub-panels: Physics (Quality dropdown, Build/Reset/Rebuild NCCs/Remove, selected RB info with Inspect/Colliders/Contacts, per-chain list with collision/physics/exclude toggles), IK Toggle (per-chain toggles + All On/Off), Animation (action name, Clear Animation).
