...
- **Materials** — two shader modes (bare Principled BSDF default, optional toon/sphere node group), specular mapping, bundled toon textures, global controls via armature properties (no drivers). Overlapping materials (e.g. eye highlights) auto-detected and use Diffuse BSDF + Transparent BSDF mix (BLENDED) to prevent z-fighting. Optional "Offset Overlapping" import checkbox nudges overlay vertices along normals for Cycles compatibility. Global `mmd_emission` on armature × per-material `mmd_emission_fac` → Emission Strength (reactive via depsgraph handler and registered property callback). Per-material shadow control via `visible_shadow` per mesh object (works because each material is its own object in split mode).
- **Additional transforms** — grant parent system (D bones, shoulder cancel, arm twist, eye tracking), shadow bones.
- **Edge outlines** — Solidify modifier + Emission BSDF, per-material edge color/size/alpha, per-vertex edge_scale, per-mesh controls. Global thickness (`mmd_edge_thickness`) stored on armature (per-model, animatable).
...
- **Material shadow/blend API**: `shadow_method` removed — shadow casting is object-level only (`obj.visible_shadow`). With per-material mesh split, `visible_shadow` is effectively per-material (each material is its own object). PMX `enabled_drop_shadow` flag is honored during mesh build. `blend_method` deprecated — use `surface_render_method` (`DITHERED` replaces `HASHED`/`OPAQUE`/`CLIP`, `BLENDED` replaces `BLEND`). `show_transparent_back` deprecated — use `use_transparency_overlap`.
